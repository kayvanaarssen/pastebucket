...
- **ChatGPT Connection** - A hosted MCP endpoint (`/mcp`) with OAuth 2.1 so ChatGPT can publish, check and revoke documents. On this route the server encrypts the content itself, which is stated on the consent screen. Connected apps can be disconnected from the profile page. See [docs/chatgpt.md](docs/chatgpt.md).
