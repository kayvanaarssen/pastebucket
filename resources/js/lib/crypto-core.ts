/**
 * End-to-end encryption for paste content -- the environment-independent core.
 *
 * Everything here runs unchanged in the browser and in Node (22+), because it
 * only touches WebCrypto, TextEncoder/TextDecoder and btoa/atob, which both
 * provide as globals. The browser page and the local MCP client import this
 * same file, so a paste published from a terminal decrypts through exactly
 * the code path the website uses. Browser-only helpers (the URL fragment,
 * sessionStorage) live in crypto.ts.
 *
...
export function toBase64Url(bytes: Uint8Array): string {
    // Chunked so a multi-megabyte transcript does not build its binary string
    // one concatenation per byte.
    let binary = '';
    const chunk = 0x8000;
...
/**
 * The exact plaintext size, in bytes, behind a base64url AES-GCM ciphertext.
 * The server enforces its payload limit with the same arithmetic, so a client
 * can refuse oversized content before it ever encrypts or sends it.
 */
export function plaintextBytesForCiphertext(ciphertext: string): number {
    return Math.max(0, Math.floor((ciphertext.length * 3) / 4) - GCM_TAG_BYTES);
}
...
export async function decryptWithCek(
    ...
    try {
        ...
        // ignoreBOM keeps a leading byte-order mark as content. The default
        // silently strips it, and a published file must come back byte for byte.
...
/**
 * Absolute share URL for a paste. The key goes in the fragment and nowhere
 * else; without it a fragment-mode link is useless.
 */
export function shareUrlFor(origin: string, slug: string, key: string | null): string {
    const base = `${origin.replace(/\/+$/, '')}/p/${encodeURIComponent(slug)}`;
    return key ? `${base}#k=${key}` : base;
}

/** True when the runtime can do the crypto we need (WebCrypto requires a secure context). */
